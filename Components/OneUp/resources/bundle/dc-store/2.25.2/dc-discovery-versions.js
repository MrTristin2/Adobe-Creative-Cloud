window["adobe_dc_sdk"] = window["adobe_dc_sdk"] || {}; window["adobe_dc_sdk"]["dc-discovery-versions"] =
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "default", function() { return discoveryDynamic; });
/* harmony import */ var semver__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(0);
...
function discoveryDynamic(discovery, origin, clientLogger, expandURL, merge, dcCore) {
  var logger = dcCore.logging.getLogger('dc-discovery'); // the parsed hostVersion (parsed once for performance)

  var hostVersion = "2.25.2".split('.').map(function (v) {
    return +v;
  });
  /**
   * @description
  ...
  function compatibleCoreVersion(host, dropin) {
    var d = dropin.split('.').map(function (v) {
      return +v;
    });
  ...
  function _findDropins() {
    var grouping;
    var vars = {};
    var components = merge(window.adobe_dc_sdk.version_manifest, discovery.overrides); // This components list comes via our version manifest
    // eslint-disable-next-line

    for (grouping in components) {
      try {
        var _ret = function () {
          var group = components[grouping];
          ...
          Object.keys(group.providers || []).forEach(function (key) {
            // make a copy of any provider vars to be used later by EnvUtil
            Object.assign(vars, group.providers[key]);
          });
          var pinnedRange = discovery.pinnedRanges && discovery.pinnedRanges[group.component]; // start with a candidate dropin.  It will have a map of core_versions
          // that point to versions of dropins built with those dc-core versions

          var versions = group.core_versions;
          var candidates = []; // Select the newest component version that satisfies the pinned version
          // Assume list is in sorted order. i.e. first match is newest

          var findNewest = function findNewest(found, curr) {
            if (found) {
              return found;
            }

            if (semver__WEBPACK_IMPORTED_MODULE_0___default.a.satisfies(curr, pinnedRange)) {
              return curr;
            }
          ...
          var findMax = function findMax(prev, curr) {
            if (prev && curr) {
              return semver__WEBPACK_IMPORTED_MODULE_0___default.a.gt(prev.component, curr.component) ? prev : curr;
            }
          ...
          var unfiltered = []; // check dc-core versions in reverse order since the most likely
          // matches are in the newer versions

          Object.keys(versions).reverse().forEach(function (v) {
            var versionCount = versions[v].length;
            ...
            if (!compatibleCoreVersion("2.25.2", v)) {
              return;
            }

            var maxComponentVersion = versions[v][versionCount - 1];
            ...
            if (pinnedRange) {
              ...
              if (semver__WEBPACK_IMPORTED_MODULE_0___default.a.ltr(maxComponentVersion, pinnedRange)) {
                return;
              } // expect to find a candidate in the list...
              // extract the newest version that satisfies the range


              var newest = versions[v].reduceRight(findNewest, null);
          ...
          if (!componentVersion && unfiltered.length) {
            clientLogger.warn("failed to find \"".concat(grouping, "\" in range: ").concat(pinnedRange)); // revert to the last deployment
          ...
          if (!componentVersion) {
            ...
            clientLogger.warn("\"".concat(grouping, "\" not found for version: ").concat("2.25.2")); // eslint-disable-next-line

            return "continue";
          }

          var version = "".concat(componentVersion.core, "_").concat(componentVersion.component);
          var expandTemplate = expandURL.bind(null, version, grouping);
          expandTemplate(group);

          var finalURL = discovery._applyOverrides(group);
        ...
        clientLogger.error("Error: \"".concat(err.toString(), "\" for component: ").concat(grouping));
      }
    }
  ...
  function _loadVersions() {
    ...
    var versionURL = '/dc-discovery/versionManifest.js'; // if running on localhost, use the version manifest from the dev server

    versionURL = origin + versionURL; // check of there's an override for the discovery manifest
    ...
    return dcCore.loadScript(versionURL).then(function () {
...
module.exports = (__webpack_require__(3))("W2nU");

/***/ }),
/* 3 */
/***/ (function(module, exports) {

module.exports = window['adobe_dc_sdk']['dc-core'];
