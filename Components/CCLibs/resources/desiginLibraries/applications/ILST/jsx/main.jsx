...
$.evalFile(extensionPath + 'material.jsx');
...
function getThumbnailOptions(maxWidth, maxHeight) {
    var options = new ExportOptionsPNG24();
    options.antiAliasing = true;
    options.transparency = true;
    var dimn = new Dimensions();
    dimn.width = maxWidth ? maxWidth : JSXGlobals.previewMaxWidth;
    dimn.height = maxHeight ? maxHeight : JSXGlobals.previewMaxHeight;
    options.dimensions = dimn;

    return options;
}

function areEqual(colorData1, colorData2) {
    return colorData1 && colorData2 && JSON.stringify(colorData1[0].value) === JSON.stringify(colorData2[0].value);
}

function pushUnique(colorData, colorType, layerColors) {
...
function populateTextOptionsInLayerObject(layerObject, selection, fillTooltipObj, strokeTooltipObj, isFontAvailableinAIObj) {
    ...
    fillTooltipObj.fillTooltip = JSXGlobals.AI_TEXT_FILL;
    strokeTooltipObj.strokeTooltip = JSXGlobals.AI_TEXT_STROKE;
    try {
...
$._ADBE_LIBS_ILST = {
    ...
    getSelectedMaterialList: function () {
        try {
            var activeDoc = app.activeDocument;
            var result = [];
            var i;
            if (activeDoc) {
                var material = activeDoc.materials;
                if (material) {
                    var materialList = material.getSelectedMaterials();
                    if (materialList && materialList.length) {
                        for (i = 0; i < materialList.length; i++) {
                            result.push({
                                'name': materialList[i].name,
                                'type': 'application/vnd.adobe.element.material+dcx',
                                'representations': MATERIAL.materialToData(materialList[i])
                            });
                        }
                    }
                }
            ...
            $._ADBE_LIBS_CORE.writeToLog('ILST.jsx-getSelectedMaterialList()', ex);
        }
    },
    ...
    getItemID: function (layer) {
        try {
            ...
            if (Object.prototype.hasOwnProperty.call(layer, "zOrderPosition")) {
                try {
    ...
    getLayerInfo: function () {
        try {
            var layerObject = {};
            var layerColors = [];
            var fillTooltipObj = { fillTooltip: JSXGlobals.FILL };
            var strokeTooltipObj = { strokeTooltip: JSXGlobals.STROKE };
            var isFontAvailableinAIObj = { isFontAvailableinAI: true };
            ...
            if (appSelection) {
                ...
                if (selection) {
                    if (selection.length === 1 && selection[0] && selection[0].__class__ === "TextFrame") {
                        ...
                        populateTextOptionsInLayerObject(layerObject, selection, fillTooltipObj, strokeTooltipObj, isFontAvailableinAIObj);
                ...
                try {
                    ...
                    pushUnique(COLOR.solidColorToData(fillColor), fillTooltipObj.fillTooltip, layerColors);
                } catch (ignore) {}

                try {
                    var strokeColor = app.activeDocument.defaultStrokeColor;
                    pushUnique(COLOR.solidColorToData(strokeColor), strokeTooltipObj.strokeTooltip, layerColors);
                } catch (ignore) {}
    ...
    saveAsPNG: function (filePath, exportFullFile, maxWidth, maxHeight) {
        try {
            var pngFile = new File(filePath);

            var options = getThumbnailOptions(maxWidth, maxHeight);
            if (exportFullFile) {
    ...
    saveAssets: function (info, generateSecondaryFormat, dragAssetId) {
        try {
            ...
            $._ADBE_LIBS_ILST.saveAsPNG(pngPath, false, info.maxWidth, info.maxHeight);
