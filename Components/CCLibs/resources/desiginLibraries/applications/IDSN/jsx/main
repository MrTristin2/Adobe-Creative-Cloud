...
$._ADBE_LIBS_IDSN = {
    setThumbnailExportOptions: function () {
        ...
        $._ADBE_LIBS_IDSN._setThumbnailDimensions(JSXGlobals.previewMaxWidth, JSXGlobals.previewMaxHeight);
    },
    setHSBSupport: COLOR.setHSBSupport,
    ...
    placeAsset: function (filePath, libraryName, itemName, elementRef, modifiedTime, creationTime, adobeStockId, adobeStockLicense, isLinked, forceInPlaceGun, isOpenInWorkflow) {
        try {
            ...
            if (appIsInTouchMode || isOpenInWorkflow) {
                ...
                if (selection.length === 1 && selection[0] && Object.prototype.hasOwnProperty.call(selection[0], 'characters')) {
                    if (selectionClass !== 'TextFrame' && selectionClass !== 'Cell' && selectionClass !== 'EndnoteTextFrame') {
    ...
    getLayerInfo: function () {
        try {
            ...
            try {
                ...
                if (!COLOR.isRestrictedColor(fillColor)) {
            ...
            try {
                ...
                if (!COLOR.isRestrictedColor(strokeColor)) {
            ...
            try {
                ...
                if (selection.length === 1 && selection[0] && Object.prototype.hasOwnProperty.call(selection[0], 'characters')) {
                    ...
                    if (selectionClass !== 'TextFrame' && selectionClass !== 'Cell' && selectionClass !== 'EndnoteTextFrame') {
                        ...
                        textObj = selection[0].parentStory;
                    }
    ...
    _currentThumbnailHeight: 0,
    _setThumbnailDimensions: function (width, height) {
        app.setCloudLibraryOptions(width, height);
        $._ADBE_LIBS_IDSN._currentThumbnailWidth = width;
        $._ADBE_LIBS_IDSN._currentThumbnailHeight = height;
    },
    saveAssets: function (info, generateSecondaryFormat, dragAssetId) {
        try {
            ...
            if (info.maxWidth > 0 && info.maxHeight > 0) {
                var width = $._ADBE_LIBS_IDSN._currentThumbnailWidth;
                var height = $._ADBE_LIBS_IDSN._currentThumbnailHeight;
                $._ADBE_LIBS_IDSN._setThumbnailDimensions(info.maxWidth, info.maxHeight);
                app.exportSelectionForCloudLibrary(idmsFile);
                $._ADBE_LIBS_IDSN._setThumbnailDimensions(width, height);
            } else {
                app.exportSelectionForCloudLibrary(idmsFile);
            }
    ...
    getColorDataFromJSON: COLOR.getColorDataFromJSON,
