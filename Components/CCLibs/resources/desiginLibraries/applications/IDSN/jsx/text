...
TEXT.saveTextStylePreview = function () {
    ...
    if (selection.__class__ === 'TextFrame' || selection.__class__ === 'Cell' || selection.__class__ === 'EndnoteTextFrame') {
...
TEXT.getFontPostscriptName = function (font, fontStyle) {
    ...
    if (postscriptName === '' && app.documents.length !== 0) {
        var fontName = fontFamily, origFontName = fontFamily;
        if (fontStyle !== '') {
            fontName += '\t' + fontStyle;
        }
        ...
        if (!docFont || !docFont.isValid) {                 //fallback
            try {
                docFont = app.activeDocument.fonts.itemByName(origFontName);
            } catch (ex) {
                docFont = app.fonts.itemByName(origFontName);
            }
        }
...
TEXT.createTypeStyle = function (typography) {
    ...
    if (Object.prototype.hasOwnProperty.call(typography, 'idmsPath')) {
...
TEXT.addTextData = function (textObj, textFrame) {
    ...
    for (paraItr = 0; paraItr < paragraphsLen; paraItr++) {
        ...
        if (!Object.prototype.hasOwnProperty.call(stylesMap, styleId)) {
        ...
        for (rangeItr = 0; rangeItr < rangesLen; rangeItr++) {
            ...
            if (rangeItr === 0 && range.textHasOverrides(StyleType.PARAGRAPH_STYLE_TYPE, false)) {
                ...
                if (Object.prototype.hasOwnProperty.call(paraOverrides, 'adbeParaAlignment') && !Object.prototype.hasOwnProperty.call(paraOverrides, 'adbeIdsnParagraphDirection')) {
                    paraOverrides.adbeIdsnParagraphDirection = stylesMap[paraObject.styles[0]].value.adbeIdsnParagraphDirection;
                } else if (!Object.prototype.hasOwnProperty.call(paraOverrides, 'adbeParaAlignment') && Object.prototype.hasOwnProperty.call(paraOverrides, 'adbeIdsnParagraphDirection')) {
...
TEXT.captureText = function (text) {
    ...
    try {
        ...
        if (text.__class__ === 'TextFrame' || text.__class__ === 'EndnoteTextFrame') {
            origTextFrame = text;
...
TEXT.applyStylesOnParagraph = function (document, paragraph, paraJSONObject, styles, styleMap) {
    try {
        ...
        if (paraJSONObject.runs) {
            ...
            var comparisonFunc = function (firstEl, secondEl) {
                if (firstEl.from < secondEl.from) {
                    return -1;
                }
                if (firstEl.from > secondEl.from) {
                    return 1;
            ...
            paraJSONObject.runs.sort(comparisonFunc);
            ...
            for (runIndex = 0; runIndex < runLength; runIndex++) {
                ...
                var parChar = paragraph.characters;
                ...
                for (styleIndex = 0; styleIndex < numStyles; styleIndex++) {
                    ...
                    if (style) {
                        ...
                        if (!styleObj.name) {
                            ...
                            isCharStyle = styleObj.type === CHARSTYLE_REPRESENTATION_JSON_TYPE;
                            if (isCharStyle) {
                                if (run.to >= parChar.length - 1) {
                                    runTo = parChar.length - 1;
                                } else {
                                    runTo = run.to - 1;
                                }
                                ...
                                if (namedCharStyleApplied === false) {
                                    ...
                                    if (parChar[run.from].isValid === false) {
                                        continue;
                                    }
                                    dummyCharStyle = document.characterStyles.add();
                                    previousStyle = parChar[run.from].appliedCharacterStyle;
                                    parChar.itemByRange(run.from, runTo).appliedCharacterStyle = dummyCharStyle;
                                    dummyCharStyleApplied = true;
                                }

                                //iterate over the ranges.
                                var parLen = paragraph.textStyleRanges.length;
                                for (rangeItr = previousIndex; rangeItr < parLen; rangeItr++) {
                                    ...
                                    if (styleIndex == numStyles-1)
                                        previousIndex = rangeItr;
                                }
                            ...
                            if (style.__class__ === 'CharacterStyle') {
                                if (runTo >= parChar.length - 1) {
                                    runTo = parChar.length - 1;
                                } else {
                                    runTo = run.to - 1;
                                }
                                parChar.itemByRange(run.from, runTo).appliedCharacterStyle = style;
...
TEXT.setTextFromJSON = function (textData, withStyles, object) {
    var setTextInternal = function (textData) {
        try {
            ...
            if (object.__class__ === 'TextFrame' || object.__class__ === 'EndnoteTextFrame') {
...
TEXT.canAddTypeStyle = function (text, isCharStyle) {
    ...
    if (allowed && text.appliedFont.__class__ === 'Font') {
        allowed = text.appliedFont.status !== FontStatus.NOT_AVAILABLE;
    }
